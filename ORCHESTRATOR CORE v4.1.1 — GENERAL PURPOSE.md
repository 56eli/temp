# ORCHESTRATOR CORE v4.1.1 — GENERAL PURPOSE
...
publish form's alignment block below (through the closing `fi`, before the
state edit and add), in its own `set -e` subshell, before copying. The same
...
- On divergence, the local working state stays untouched and the fetched pin
  ...
  state only after alignment. The unique scratch backup is a local recovery
  aid, not durable memory or a substitute for publishing the reconciled state.
  ...
  if git ls-remote --exit-code --heads origin refs/heads/<ORCHESTRATOR_BRANCH>; then
    ...
    if git merge-base --is-ancestor refs/remotes/origin/_orch HEAD; then
      ...
      backup=$(mktemp -d /tmp/orch-pub.XXXXXX)
...
1. Confirm you have loaded this orchestrator prompt (v4.1.1).
